...
def _validate_handoff_contract(value: dict[str, Any]) -> None:
    """Keep five-file authority, product ownership, and future-provider choices explicit."""

    if (
        value.get("id") != "pi-five-file-authority"
    ...
    if source_files != sorted(BUNDLE_FILES):
        raise ConfigError("handoff authority must name exactly the five source files")
    if authority.get("exact_source_file_count") != len(BUNDLE_FILES):
        raise ConfigError("handoff authority source file count must remain five")
    ...
    if product.get("brand") != "PI" or product.get("assistant") != "π Gen":
        raise ConfigError("handoff product brand must remain PI with π Gen")
    if (
    ...
    if (
        ...
        raise ConfigError("π Gen runtime provider must remain unselected and local-mock")
    if (
        ...
        raise ConfigError("π Gen provider or secret boundary is invalid")
    ...
    if not isinstance(candidates, list) or {
        ...
        raise ConfigError("π Gen future provider candidates are incomplete")
    if any(
        ...
        raise ConfigError("all π Gen future provider candidates must remain disabled and unselected")
